 ...
 *   LONG_AMOUNT: Long token 数量，单位为 token（可选，默认 1）
 *   SHORT_AMOUNT: Short token 数量，单位为 token（可选，默认 5000）
 *
 * 示例:
 *   MARKET_ADDRESS=0x... LONG_AMOUNT=100 SHORT_AMOUNT=500000 make user-deposit
 *   # 存入 100 BTC + 500,000 USDC
 *
 * 脚本会自动从 Market 获取正确的 long/short token 地址和 decimals
 */
...
async function main() {
  ...
  // 设置金额
  // LONG_AMOUNT: 单位为 token (如 100 表示 100 BTC)，默认 1
  const longAmountUnits = process.env.LONG_AMOUNT ? parseFloat(process.env.LONG_AMOUNT) : 1;
  const longTokenAmount = ethers.utils.parseUnits(longAmountUnits.toString(), longDecimals);

  // SHORT_AMOUNT: 单位为 token (如 500000 表示 500,000 USDC)，默认 5000
  const shortAmountUnits = process.env.SHORT_AMOUNT ? parseFloat(process.env.SHORT_AMOUNT) : 5000;
